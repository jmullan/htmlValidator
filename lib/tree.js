...
Tree.prototype = {
    ...
    insertElement: function (token) {
        ...
        if (typeof token === 'string') {
            prev = _.last(this.stack).content;
            if (typeof _.last(prev) === 'string') {
                prev[prev.length - 1] = _.last(prev) + token;
                return token;
            }
    ...
    'in body': function (token) {
        ...
        case 'StartTag':
            ...
            return;
        case 'EndTag':
            if ('body' === token.name && !this.hasElementInScope('body')) {
                this.errors.push(new errors.ParseError(token));
                return;
            }

            if ('body' === token.name) {
                // TODO: stack of open elements has a node that is not one of these? parse error
                // if (_.include(['dd', 'dt', 'li', 'optgroup', 'option', 'p', 'rp', 'rt', 'tbody', 'td', 'tfoot', 'th', 'th', 'tr', 'body', 'html']))
