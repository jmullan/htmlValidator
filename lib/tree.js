...
Tree.prototype = {
    ...
    'in body': function (token) {
        if (!token || typeof token === 'string') {
            if (!token || (/\0/).test(token)) { // null character?
                this.errors.push(new errors.ParseError(token));
                return;
            }
        ...
        function endPreviousElements(tags) {
            var node = _.last(self.stack),
                i = 0;
            if (_.include(tags, node.name)) {
                self['in body']({ name: node.name, type: 'EndTag' });
            } else {
                i = 0;
