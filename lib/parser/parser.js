...
Tree.prototype = {
    ...
    process: function (mode, token) {
        if (!_.find(['initial', 'before html', 'before head', 'in head'], function (val) { return val === mode; })) {
    ...
    'in head noscript': modes['in head noscript'].mode,

    'after head': modes['after head'].mode,

    'in body': modes['in body'].mode,

    text: modes.text.mode,

    'in table': modes['in table'].mode,

    'in table text': modes['in table text'].mode,

    'in caption': modes['in caption'].mode,

    'in column group': modes['in column group'].mode,

    'in table body': modes['in table body'].mode,

    'in row': modes['in row'].mode,

    'in cell': modes['in cell'].mode,

    'in select': modes['in select'].mode,

    'in select in table': modes['in select in table'].mode,

    'after body': modes['after body'].mode,

    'in frameset': modes['in frameset'].mode,

    'after frameset': modes['after frameset'].mode,

    'after after body': modes['after after body'].mode,

    'after after frameset': modes['after after frameset'].mode
};
exports.Tree = Tree;
